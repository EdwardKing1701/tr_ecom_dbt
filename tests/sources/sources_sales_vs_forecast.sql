...
cte_test as (
    select
        ...
        (select (select sum(forecast) from {{ref('v_fct_forecast_by_day')}} where date = current_date() - 1) / (select sum(sale_amt) from {{ref('v_fct_order_items')}} where date = current_date() - 1) - 1) as data,
