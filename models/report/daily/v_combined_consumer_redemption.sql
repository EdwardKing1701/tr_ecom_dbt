...
f_orders_combined AS (
    ...
    SELECT
     ...
     -- SFCC_CUSTOMER_ID,
      ...
      LOYALTY_OPTOUT_DT,
      CASE WHEN
        IS_LOYALTY_TRANSACTION = 'true' THEN ORDER_NET_TOTAL ELSE NULL END AS LOYALTY_NET_TOTAL,
      CASE
        WHEN IS_LOYALTY_MEMBER IS NOT NULL AND LOYALTY_TIER IS NULL THEN 'Insider'
        ELSE LOYALTY_TIER END AS LOYALTY_TIER,
      LOYALTY_REDEMPTION_FLAG,
      CASE WHEN
        LOYALTY_REDEMPTION_FLAG = 1 THEN ORDER_NET_TOTAL ELSE NULL END AS REDEMPTION_NET_TOTAL,
      CASE WHEN
        LOYALTY_REDEMPTION_FLAG = 1 THEN ORDER_NET_TOTAL ELSE RT.ORDER_ID END AS REDEMPTION_TRANSACTION_ID,
      RCD_INS_TS,
      RCD_UPD_TS
    FROM
      COMBINED
